...
# Удаляем все старые Received
/^Received:.*$/ IGNORE

# Добавляем новый Received в самое начало письма (перед всеми заголовками)
/^Message-ID:/ PREPEND Received: from [${DOMAIN}] by ${HOSTNAME}
...
install -d -m 0750 -o opendmarc -g postfix /var/spool/postfix/opendmarc
