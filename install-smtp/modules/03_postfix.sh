...
# Общие настройки milter
postconf -e \
  "milter_protocol=6" \
  "milter_default_action=accept"

# Входящий SMTP на 25/tcp (chroot): DKIM + DMARC
# Внутри chroot Postfix видит сокеты как /opendkim/... и /opendmarc/...
postconf -e \
  "smtpd_milters=unix:/opendkim/opendkim.sock,unix:/opendmarc/opendmarc.sock"

# Исходящее не через smtpd (pickup/local/pipe): только DKIM (полный путь вне chroot)
postconf -e \
  "non_smtpd_milters=unix:/var/spool/postfix/opendkim/opendkim.sock"

# MSA-порты (587/465): аутентифицированный исходящий — только DKIM
postconf -P 'submission/inet/smtpd_milters=unix:/var/spool/postfix/opendkim/opendkim.sock'
postconf -P 'smtps/inet/smtpd_milters=unix:/var/spool/postfix/opendkim/opendkim.sock'

# --- Чистка «лишних» заголовков исходящих писем ---
# Экранируем HOSTNAME для regex
HOSTNAME_RE="$(printf '%s\n' "$HOSTNAME" | sed -e 's/[.[*^$(){}|+?\\]/\\&/g')"

cat > /etc/postfix/smtp_header_checks <<EOF
# Убираем свои локальные Received, если вдруг проскакивают
/^Received: from (localhost|\[127\.0\.0\.1\])\b/         IGNORE
/^Received: from .*\.lan\b/                               IGNORE

/^Received:.*$/ IGNORE
/^Message-ID:/ PREPEND Received: from [cleaned] by mail.drochimvmeste.ru

# Прячем IP/клиентский софт
/^X-Originating-IP:/                                      IGNORE
/^User-Agent:/                                            IGNORE
/^X-Mailer:/                                              IGNORE

# Не светим внутреннее имя нашего MTA
/^Received: from ${HOSTNAME_RE}\b/                        IGNORE
EOF
...
INFO "Postfix: virtual_* (контроль)"
postconf | egrep '^(virtual_mailbox_base|virtual_mailbox_maps|virtual_mailbox_domains|virtual_transport|virtual_uid_maps|virtual_gid_maps)' || true
INFO "Postfix: ключевые параметры (контроль)"
postconf -n | egrep -i '^(inet_protocols|smtp_address_preference|smtp_bind_address|mynetworks|smtpd_helo_required|disable_vrfy_command|smtpd_delay_reject|smtpd_(relay|recipient|sender|client)_restrictions|postscreen_|anvil_|smtpd_tls_security_level|smtp_tls_security_level|smtpd_milters|non_smtpd_milters|milter_protocol|milter_default_action|smtp_header_checks)$' || true
